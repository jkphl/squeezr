...
class Css extends \Tollwerk\Squeezr
{
    ...
    public function send($cache = true)
    {
        ...
        try {
            ...
            if ((!@is_dir($this->_absoluteCacheCssDir) && !@mkdir($this->_absoluteCacheCssDir, 0777,
                        true)) || !@is_writable($this->_absoluteCacheCssDir)
            ) {
            ...
            } elseif (empty($_COOKIE['squeezr_css']) || !preg_match("%^(\d+)x(\d+)\@(\d+(?:\.\d+)?)$%",
                    $_COOKIE['squeezr_css'], $squeezr)
            ) {
                ...
                if (@is_file($this->_absoluteCachePhpPath)) {
                    ...
                    if ($cache && !@file_put_contents($this->_absoluteCachePhpPath, $cacheClassCode)) {
                        $this->_addErrorHeader(sprintf(\Tollwerk\Squeezr\Exception::FAILED_WRITING_CACHE_FILE_STR,
                            $this->_absoluteCachePhpPath), \Tollwerk\Squeezr\Exception::FAILED_WRITING_CACHE_FILE);
                ...
                if (is_object($cacheInstance)) {
                    ...
                    if ($cache) {
                        ...
                        if (!@is_file($breakpointsCachePath) && !@file_put_contents($breakpointsCachePath,
                                $returnCss)
                        ) {
                            $this->_addErrorHeader(sprintf(\Tollwerk\Squeezr\Exception::FAILED_WRITING_CACHE_FILE_STR,
                                $breakpointsCachePath), \Tollwerk\Squeezr\Exception::FAILED_WRITING_CACHE_FILE);
                            ...
                            $this->_addErrorHeader(sprintf(\Tollwerk\Squeezr\Exception::FAILED_WRITING_CACHE_FILE_STR,
                                $this->_absoluteCacheCssPath), \Tollwerk\Squeezr\Exception::FAILED_WRITING_CACHE_FILE);
    ...
    protected function __construct($css)
    {
        ...
        $this->_absoluteCssPath = rtrim(SQUEEZR_DOCROOT,
                DIRECTORY_SEPARATOR).DIRECTORY_SEPARATOR.$this->_relativeCssPath;
        ...
        if (!@is_readable($this->_absoluteCssPath)) {
            throw new \Tollwerk\Squeezr\Exception(sprintf(\Tollwerk\Squeezr\Exception::INVALID_CSS_FILE_MSG,
                $this->_absoluteCssPath), \Tollwerk\Squeezr\Exception::INVALID_CSS_FILE);
        }
        ...
        while ($peek < $length) {
            ...
            if (preg_match("%^(\s+)%isSu", $css, $whitespace)) {
                ...
                if (!strncmp('/*', $css, 2)) {
                    ...
                    if ($commentEnd === false) {
                        throw new \Tollwerk\Squeezr\Exception(sprintf(\Tollwerk\Squeezr\Exception::UNBALANCED_BLOCK_COMMENT_STR,
                            $this->_peekToLine($peek)), \Tollwerk\Squeezr\Exception::UNBALANCED_BLOCK_COMMENT);
                    }
                    ...
                    if (!preg_match("%^\@([\w\-]+)%", $css, $atRule)) {
                        throw new \Tollwerk\Squeezr\Exception(sprintf(\Tollwerk\Squeezr\Exception::INVALID_AT_RULE_IDENTIFIER_STR,
                            $this->_peekToLine($peek)), \Tollwerk\Squeezr\Exception::INVALID_AT_RULE_IDENTIFIER);
                    }
                    ...
                    switch ($atRule[1]) {
                        ...
                        case 'import':
                            ...
                            foreach (self::$_importPatterns as $start => $pattern) {
                                if (!strncmp($start, $import, strlen($start)) && preg_match($pattern, $import,
                                        $importMediaQuery)
                                ) {
    ...
    protected function _consumeDeclarationBlock($css, $peek, $offset = 0, &$declarationBlockStart = 0)
    {
        ...
        if ($declarationBlockStart === false) {
            throw new \Tollwerk\Squeezr\Exception(sprintf(\Tollwerk\Squeezr\Exception::INVALID_DECLARATION_BLOCK_STR,
                $this->_peekToLine($peek)), \Tollwerk\Squeezr\Exception::INVALID_DECLARATION_BLOCK);
        }
        ...
        while ($declarationBlockBalance > 0) {
            ...
            if (preg_match("%[\{\}]%", $css, $declarationBlockDelimiter, PREG_OFFSET_CAPTURE,
                $declarationBlockPosition)) {
                $declarationBlockBalance += ($declarationBlockDelimiter[0][0] == '{') ? 1 : -1;
                $declarationBlockPosition = $declarationBlockDelimiter[0][1] + 1;

                // Else: error
            } else {
                throw new \Tollwerk\Squeezr\Exception(sprintf(\Tollwerk\Squeezr\Exception::UNBALANCED_DECLARATION_BLOCK_STR,
                    $this->_peekToLine($peek)), \Tollwerk\Squeezr\Exception::UNBALANCED_DECLARATION_BLOCK);
            }
        }
    ...
    protected function _breakpoints($mediaQuery)
    {
        ...
        foreach (explode(',', $mediaQuery) as $mediaQueryAlternative) {
            ...
            if (preg_match_all("%((?:(?:min|max)\-)?(device\-)?width)\s*\:\s*(\-?\d+(?:\.\d+)?)([acdehimnoprtvwxz]+)%i",
                $mediaQueryAlternative, $widthConditions, PREG_SET_ORDER)) {
                foreach ($widthConditions as $widthCondition) {
                    $minWidth =& $minConditions[self::CONDITION_WIDTH];
                    $minWidth[$widthCondition[4]] = empty($minWidth[$widthCondition[4]]) ? floatval($widthCondition[3]) : min(floatval($widthCondition[3]),
                        $minWidth[$widthCondition[4]]);
                    unset($minWidth);
                }
            }

            // Detect a minimum height condition
            if (preg_match_all("%((?:(?:min|max)\-)?(device\-)?height)\s*\:\s*(\-?\d+(?:\.\d+)?)([acdehimnoprtvwxz]+)%i",
                $mediaQueryAlternative, $heightConditions, PREG_SET_ORDER)) {
                foreach ($heightConditions as $heightCondition) {
                    $minHeight =& $minConditions[self::CONDITION_HEIGHT];
                    $minHeight[$heightCondition[4]] = empty($minHeight[$heightCondition[4]]) ? floatval($heightCondition[3]) : min(floatval($heightCondition[3]),
                        $minHeight[$heightCondition[4]]);
                    unset($minHeight);
                }
            }
    ...
    protected function _breakpointIndex($type, array $condition)
    {
        ...
        if (!array_key_exists($serializedCondition, $this->_breakpointCatalog[$type])) {
            ...
            $this->_breakpointIndex[$this->_breakpointCatalog[$type][$serializedCondition]] = array(
                self::$_breakpointTypes[$type],
                $condition
            );
        }
    ...
    protected function _compileCacheClassCode()
    {
        ...
        $cacheClassCode = strtr(file_get_contents(__DIR__.DIRECTORY_SEPARATOR.'Css'.DIRECTORY_SEPARATOR.'Cache.php'),
            array(
